...
public class SpringApplication {
    ...
    public ConfigurableApplicationContext run(String... args) {
        ...
        //3、设置系统属性“java.awt.headless”的值，默认为true，用于运行headless服务器，进行简单的图像处理，多用于在缺少显示屏、键盘或者鼠标时的系统配置，
        // 很多监控工具如jconsole 需要将该值设置为true
        configureHeadlessProperty();
        //4、创建所有spring运行监听器并发布应用启动事件，简单说的话就是获取SpringApplicationRunListener类型的实例（EventPublishingRunListener对象），
        // 并封装进SpringApplicationRunListeners对象，然后返回这个SpringApplicationRunListeners对象。说的再简单点，getRunListeners就是准备好了
        // 运行时监听器EventPublishingRunListener。
    ...
    protected void logStartupProfileInfo(ConfigurableApplicationContext context) {
        ...
        if (log.isInfoEnabled()) {
            ...
            if (ObjectUtils.isEmpty(activeProfiles)) {
                ...
                log.info("No active profile set, falling back to default profiles: " + StringUtils
